...
def coefficientmatrix(n):
    ...
    for mm in range(0,n-1):#左边界
        ...
        A[n0-1,n0-n-3]=0
        return A
def fij(n,h,x,y,x_low,x_up,dy_left,dy_right):
...
h=(b_x-a_x)/n
x_cood=np.arange(a_x-h/2.,b_x+h,h)
#n+2个网格，n+2个未知数
y_cood=np.arange(a_x,b_x+h,h)
#y有n+1个点，n-1个未知数
x_low=0.
x_up=0.
dy_left=0.
...
b_matrix=fij(n,h,x_cood,y_cood,x_low,x_up,dy_left,dy_right)
#计算等式右端矩阵b
f_1d=cg(A0,b_matrix,uij)#调用共轭向量法
result=f_1d.reshape(n-1,n+2)#转换成二维矩阵
plt.figure()
y_cood2=np.arange(a_x+h,b_x,h)#y有n+1个点，n-1个未知数
print(y_cood2.shape)
